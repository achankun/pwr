...
print_centered "Created by ACHAN NODE" $GREEN
print_line
echo ""
...
install_pwr() {
    ...
    wget https://github.com/pwrlabs/PWR-Validator/releases/download/13.0.0/validator.jar -O validator.jar
...
update_pwr() {
    ...
    if [ "$choice" -eq 1 ]; then
        ...
        rm -rf config.json validator.jar blocks rocksdb
        wget https://github.com/pwrlabs/PWR-Validator/raw/refs/heads/main/config.json
        wget https://github.com/pwrlabs/PWR-Validator/releases/download/13.2.1/validator.jar
        systemctl stop pwr
        pkill -f java
        systemctl start pwr
